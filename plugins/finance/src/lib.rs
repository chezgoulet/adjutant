...
//! (SPEC §7.5) and `finance.receipts` (migration 2). Vocabulary that reaches an
...
/// Migration 2's SQL, embedded at compile time from `migrations/002_receipts.sql`.
///
/// It is a **new version**, never an edit to version 1: the core records applied
/// versions per schema and skips one it has already run without comparing its
/// SQL (`server/src/db.rs`, `server/src/plugin_runtime.rs`), so an amended
/// version 1 would be invisible on every deployed database. It adds
/// `finance.receipts` — numbered, addressed, with the troop's own wording
...
const MIGRATION_RECEIPTS: &str = include_str!("../migrations/002_receipts.sql");
...
impl AdjutantPlugin for FinancePlugin {
    ...
    fn migrations(&self) -> Vec<Migration> {
        vec![
            ...
            Migration::new(2, "receipts", MIGRATION_RECEIPTS),
        ]
    }
