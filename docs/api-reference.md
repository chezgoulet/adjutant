...
| POST | `/api/finance/transfer` | `finance:write` (troop) | `{from_fund_id\|from_fund_code, to_fund_id\|to_fund_code, amount_cents\|amount, description?, occurred_on?, fiscal_year?, allow_overdraft?, external_ref?}` — each leg is named by finance's id **or by its code**, exactly one of the two, and a code is resolved inside the transfer's own statement, so a producer that holds only codes moves money without reading finance first. Both legs land under one `transfer_group`, or neither; a fund that cannot be resolved is a `404` naming the reference, the two references resolving to one fund (or two equal ids/codes) is a `400`, and an unauthorised overdraft is a `409`. `external_ref` is the transfer's own idempotency key — one money move, not two rows — and a retry carrying a reference the ledger already holds is answered `200 {duplicate: true}` with the transfer that already exists rather than a second pair of legs. Publishes `finance.transfer.recorded` |
...
| POST | `/api/finance/dues/assess` | `finance:manage_dues` (troop) | `{member_id, tier, fiscal_year?, base_cents?, lodge_id?, status?: assessed\|self_reported\|waived, note?}` — the treasurer's route. A waiver (`status: "waived"`) assesses **its tier** and funds the whole of it from `scholarship`: the answer and the row carry `funded_cents` (what `scholarship` covers) and the draw's state (`draw_status`, `draw_ref`), so "waived, and here is who paid for it" is stated rather than a price of zero. A caller who also holds `finance:write` has the draw booked in the same call **as themselves** — one balanced transfer from `scholarship` into the configured dues fund, keyed `dues:{fiscal_year}:{member_id}` so a retry cannot double-post; a caller who does not records the draw as `unbooked` for a `finance:write` holder to book. `409` when the draw is already **booked** and the funding would change: the draw's key is deterministic, so a different amount cannot be booked under it, and the row must not be left claiming a subsidy the ledger did not move — the reason names the transfer group and the act that settles the difference. Publishes `finance.dues.assessed` |
| POST | `/api/finance/dues/repair-waivers` | `finance:manage` (troop) | `{fiscal_year?, dry_run?}` — recompute the rows **already waived at zero** (the historic rows migration 2 leaves alone) from each row's own `base_cents` and `tier`. Guarded and idempotent, run on demand and never at startup: a repair of a past year changes what that year's report says. A `base_cents = 0` row has nothing to fund and is left alone. The recomputed draw is left `unbooked` for a `finance:write` holder to book |
...
| GET | `/api/finance/dues?fiscal_year=&lodge_id=&tier=&status=` | `finance:read_all` (troop) | Every member assessment for the year with `funded_cents` and the draw's state off the row, and `paid_cents`/`outstanding_cents` derived from the ledger, totalled and grouped by tier (the totals include what `scholarship` funded and how many draws are unbooked), capped at 500 rows |
