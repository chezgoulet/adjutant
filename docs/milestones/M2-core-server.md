...
`cargo test --workspace` → **28 passed / 0 failed at commit `0cb6fff`** (62 today),
live probes → **67/67**, four batches (m1_regression, middleware, lifecycle,
...
| # | Criterion | Evidence |
|---|-----------|----------|
| 1 | Plugin registry: load, enable, disable, uninstall, hot-reload | lifecycle batch (L1–L19b): disable → route 404 (identical to a library that is absent from disk) + core routes unaffected + DB `enabled=false`; enable → 200; uninstall → 404, rows/schema survive (`data archived`), `retired_libraries ≥ 1`; reload → `reloaded: []` while uninstalled; clear the flag + reload → `reloaded: ["hello"]` with the route and its subscription live again |
